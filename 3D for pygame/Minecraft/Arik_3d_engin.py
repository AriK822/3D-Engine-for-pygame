...
class Cube(object):
    def __init__(self, center = [0, 0, 0], edge_size = 5, block_type = 'grass', face_colors = [], edge_colors = []):
        self.is_shadow = False
        self.vertecies = []
        self.og_vertecies = []
        self.center = center
        ...
        self.face_direction = ['x-', 'x+', 'z+', 'z-', 'y+', 'y-']
        
        if block_type == 'grass':
            self.face_colors = [(102, 51, 0), (102, 51, 0), (102, 51, 0), (102, 51, 0), (0, 102, 0), (77, 40, 0)]
        else:
            if not face_colors:
                self.face_colors = [(randint(0, 255), randint(0, 255), randint(0, 255)) for _ in self.faces]
            elif block_type(face_colors) == tuple:
                self.face_colors = [face_colors for _ in self.faces]
            else:
                self.face_colors = face_colors
...
class World(object):
    ...
    def apply_render_priority(self):
        ...
        face_directions = self.face_directions + [0 for _ in self.rendered_edges]
        ...
        for z in f_e_deapth[::-1]:
            ...
            new_face_directions.append(face_directions[look_up[z][0]])
            del look_up[z][0]
        ...
        self.rendered_face_directions = new_face_directions
        ...
        for obj in self.objects:
            ...
            for n, face in enumerate(obj.faces):
                if on_screen_vertecies.intersection(face):
                    ...
                    self.face_directions.append(obj.face_direction[n])
